...
void FluxObserveInit(void)
{
    ...
    struFluxOB_Param.nCurrLoopIniKIL = P_CURRENT_KI_H;      /* 电流环初始KI */

	if(UserSys.Motorset ==0)
	{
		struMotorParam.wMotorRs = (s32)(U_MOTOR_RS*10000); /* 电机内阻 单位：0.0001欧姆 */     \
		struMotorParam.wMotorLd = (s32)U_MOTOR_LD;          /* 电机D轴电感 单位：uH */          \
		struMotorParam.wMotorLd = (s32)U_MOTOR_LQ;          /* 电机Q轴电感 单位：uH */ 
		struFluxOB_Param.nSTO_KP_H = PLL_KP_GAIN_L;
		struFluxOB_Param.nSTO_KI_H = PLL_KI_GAIN_L;
	}
	else if(UserSys.Motorset ==1)
	{
		struMotorParam.wMotorRs = (s32)(U_MOTOR_RS1*10000); /* 电机内阻 单位：0.0001欧姆 */     \
		struMotorParam.wMotorLd = (s32)U_MOTOR_LD1;          /* 电机D轴电感 单位：uH */          \
		struMotorParam.wMotorLd = (s32)U_MOTOR_LQ1;          /* 电机Q轴电感 单位：uH */ 
		struFluxOB_Param.nSTO_KP_H = PLL_KP_GAIN_L1;
		struFluxOB_Param.nSTO_KI_H = PLL_KI_GAIN_L1;
	}
	...
	m_FluxOBSPara.nObsFactor = PWM_FREQ_HI;
